...
final class LoginController: RouteCollection {
    ...
    func login(_ req: Request) throws -> Future<User.LoginResponse> {
        ...
        return query.flatMap(to: User.LoginResponse.self) { user in
            ...
            if try hasher.verify(message: loginRequest.password, matches: user.password) {
                return try user.token.query(on: req).first().map(to: User.LoginResponse.self) { token in
                    if let token = token {
                        return try user.loginResponse(token: token.token)
                    } else {
                        let uuid = UUID().uuidString
                        let _ = try Token(token: uuid, user_id: user.requireID()).save(on: req)
                        
                        return try user.loginResponse(token: uuid)
                    }
                }
